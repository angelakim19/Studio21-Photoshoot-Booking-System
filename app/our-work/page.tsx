...
const arrowBtn = "absolute top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-all duration-300 w-11 h-11 rounded-full border border-[#C8A96A]/60 bg-black/30 backdrop-blur-sm text-[#C8A96A] hover:bg-[#C8A96A] hover:text-white hover:border-[#C8A96A] hover:scale-110 flex items-center justify-center shadow-lg"
...
export default function FeaturedPage() {
  ...
  const nextFeatured = () => setCurrent((prev) => (prev + 1) % images.length)
  const prevFeatured = () => setCurrent((prev) => (prev - 1 + images.length) % images.length)
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setIndexes((prev) => prev.map((i, idx) => (i + 1) % galleries[idx].length))
    }, 3000)
    return () => clearInterval(interval)
  }, [])
  ...
  const nextGallery = (i: number) => {
    setIndexes((prev) =>
      prev.map((val, idx) => idx === i ? (val + 1) % galleries[i].length : val)
    )
  }

  const prevGallery = (i: number) => {
    setIndexes((prev) =>
      prev.map((val, idx) => idx === i ? (val - 1 + galleries[i].length) % galleries[i].length : val)
    )
  }
    ...
    <div className="bg-white text-black">
      ...
      <section className="py-24 bg-[#F5F5F5]">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl md:text-4xl font-serif mb-10">Featured Work</h2>
          ...
          <div className="max-w-5xl mx-auto relative group">
            ...
            {/* LEFT ARROW */}
            <button onClick={prevFeatured} className={`${arrowBtn} left-4`}>
              <ChevronLeft />
            </button>

            {/* RIGHT ARROW */}
            <button onClick={nextFeatured} className={`${arrowBtn} right-4`}>
              <ChevronRight />
            </button>
          ...
          <div className="grid md:grid-cols-3 gap-x-10 gap-y-16">
            {galleries.map((gallery, i) => (
              <div key={i} className="relative group">
                ...
                {/* LEFT */}
                <button onClick={() => prevGallery(i)} className={`${arrowBtn} left-3`}>
                  <ChevronLeft />
                </button>

                {/* RIGHT */}
                <button onClick={() => nextGallery(i)} className={`${arrowBtn} right-3`}>
                  <ChevronRight />
                </button>
                  ...
                  {gallery.map((_, dotIndex) => (
                    <div
                      ...
                      className={`w-2 h-2 rounded-full transition-all duration-300 ${
                        dotIndex === indexes[i] ? "bg-[#C8A96A] scale-110" : "bg-white/40"
                      }`}
                    />
                  ))}
                </div>
      ...
      <section className="py-24 bg-[#1a1a1a] relative overflow-hidden">
        <div className="absolute inset-0 opacity-10">
          <Image src="/images/studio.jpg" alt="" fill className="object-cover" />
        </div>
