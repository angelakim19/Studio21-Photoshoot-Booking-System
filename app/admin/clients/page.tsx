...
export default function ClientsPage() {
  ...
  const handleEditClient = async () => {
    ...
    if (error) {
      alert(error.message.includes("duplicate") ? "Email already exists." : "Failed to update client.")
      return
    }
    ...
    <div className="p-6 space-y-6">
      ...
      {search && (
        <p className="text-sm text-gray-500">
          Showing <span className="font-semibold text-gray-700">{data.length}</span> result{data.length !== 1 ? "s" : ""} for{" "}
                    ...
                    <td className="p-4">
                      <div className="flex gap-3 text-sm">
                        <button
                          ...
                          className="text-[#C8A96A] hover:text-[#b8935a] transition-colors"
                          title="Edit"
                        >
                          <Pencil size={16} />
                        </button>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 overflow-y-auto py-4">
          <div className="bg-white w-[600px] max-h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="border-t bg-[#F5F5F5] px-6 py-3 flex justify-end gap-2">
              <button onClick={() => setOpenAddDialog(false)} className="px-3.5 py-2 text-[#6B6B6B] hover:text-[#1A1A1A] transition-colors text-sm font-medium">
                Cancel
              </button>
              <button
      ...
      {openEditDialog && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 overflow-y-auto py-4">
          <div className="bg-white w-[600px] max-h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="border-t bg-[#F5F5F5] px-6 py-3 flex justify-end gap-2">
              <button onClick={() => setOpenEditDialog(false)} className="px-3.5 py-2 text-[#6B6B6B] hover:text-[#1A1A1A] transition-colors text-sm font-medium">
                Cancel
              </button>
              <button
      ...
      {openDeleteDialog && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 relative shadow-lg">
            ...
            <h2 className="text-center text-lg font-semibold text-gray-800">Confirm Deletion</h2>
            ...
            <div className="flex gap-3 mt-6">
              <button
                onClick={() => setOpenDeleteDialog(false)}
                className="flex-1 px-4 py-2.5 rounded-xl text-sm font-medium bg-gray-100 text-gray-700 border border-gray-200 hover:bg-gray-200 hover:border-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-300 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Cancel
              </button>
              <button
                onClick={handleDelete}
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold bg-red-500 text-white hover:bg-red-600 active:scale-[0.98] focus:outline-none focus:ring-2 focus:ring-red-400 focus:ring-offset-1 transition-all disabled:opacity-70 disabled:cursor-not-allowed shadow-sm shadow-red-200"
              >
                {loading ? (
                  <>
